...
pub fn count_xmas(m: &Matrix, x: usize, y: usize, val: char) -> anyhow::Result<(u64, Vec<(usize, usize, char)>)> {
    ...
    if x >= 3 && y >= 3 {
        possible_options.push(vec![(x-1, y-1, 'M'), (x-2, y-2, 'A'), (x-3, y-3, 'S')]);
    }

    if x >= 3 {
        possible_options.push(vec![(x-1,y, 'M'), (x-2,y, 'A'), (x-3,y, 'S')]);
        possible_options.push(vec![(x-1, y+1, 'M'), (x-2, y+2, 'A'), (x-3, y+3, 'S')]);
    }

    if y >= 3 {
        possible_options.push(vec![(x, y-1, 'M'), (x, y-2, 'A'), (x, y-3, 'S')]);
        possible_options.push(vec![(x+1, y-1, 'M'), (x+2, y-2, 'A'), (x+3, y-3, 'S')]);
    }
