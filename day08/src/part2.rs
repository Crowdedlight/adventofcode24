...
pub fn process(input: &str) -> anyhow::Result<String> {
    ...
    for (_, val) in hmap.iter() {
        ...
        for p in val.iter() {
            ...
            for p2 in val.iter() {
                ...
                // while still in board, keep trying next pos
                let mut curr_x = p2.x;
                let mut curr_y = p2.y;
                loop {
                    ...
                    match matrix.get(curr_x, curr_y) {
                        Some(c) => {antinodes_pos.insert(Pos {x: curr_x, y: curr_y});},
                        None => {break}
                    }
                }
            }
    ...
    for pos in antinodes_pos.iter() {
            matrix.set(pos.x as usize, pos.y as usize, '#');
    }
    matrix.print();
